...
def get_iou(det, gt): # tinh iou tren detection box va gt box
    det_x1, det_y1, det_x2, det_y2 = det
    gt_x1, gt_y1, gt_x2, gt_y2 = gt
    
    x_left = torch.max(det_x1, gt_x1)
    y_top = torch.max(det_y1, gt_y1)
    x_right = torch.min(det_x2, gt_x2)
    y_bottom = torch.min(det_y2, gt_y2)
...
def compute_map(det_boxes, gt_boxes, iou_threshold=0.5, method='area'): # tinh mAP
    # det_boxes = [
    #   {
    ...
    gt_labels = {cls_key for im_gt in gt_boxes for cls_key in im_gt.keys()} # Lấy danh sách tất cả các class có mặt trong ground truth
    ...
    for idx, label in enumerate(gt_labels):
        ...
        ] # Gom các box dự đoán thuộc class đang xét
        ...
        # Sort theo conf score giam dan
        cls_dets = sorted(cls_dets, key=lambda k: -k[1][-1])
        
        # Theo doi cac gt box cua class nay da duoc match chua
        gt_matched = [[False for _ in im_gts[label]] for im_gts in gt_boxes]
        # Number of gt boxes for this class for recall calculation
        num_gts = sum([len(im_gts[label]) for im_gts in gt_boxes])
        tp = [0] * len(cls_dets) # TRUE POS
        fp = [0] * len(cls_dets) # FALSE POS
...
def load_model_and_dataset():
    ...
    voc = VOCDataset('test', im_dir='VOC2007-test/JPEGImages', ann_dir='VOC2007-test/Annotations')
    test_dataset = DataLoader(voc, batch_size=1, shuffle=False)
    
    faster_rcnn_model = FasterRCNN(num_classes=21)
    faster_rcnn_model.eval()
    faster_rcnn_model.to(device)
    faster_rcnn_model.load_state_dict(torch.load(os.path.join('voc',
...
def infer():
    ...
    for sample_count in tqdm(range(10)): # lap qua 10 anh test ngau nhien
        random_idx = random.randint(0, len(voc))
        
        im, target, fname = voc[random_idx] # target[bboxes], target[labels] (GTBOX)
        im = im.unsqueeze(0).float().to(device)

        gt_im = cv2.imread(fname) # doc anh goc bang opencv 
        gt_im_copy = gt_im.copy()
        
        # save ảnh với gt boxes
        for idx, box in enumerate(target['bboxes']):
            x1, y1, x2, y2 = box.detach().cpu().numpy()
            x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
            
            cv2.rectangle(gt_im, (x1, y1), (x2, y2), thickness=2, color=[0, 255, 0]) # Vẽ box
        ...
        cv2.imwrite('samples/output_frcnn_gt_{}.png'.format(sample_count), gt_im) # lưu ảnh
